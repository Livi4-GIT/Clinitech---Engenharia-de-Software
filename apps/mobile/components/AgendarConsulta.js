...
export default function AgendarConsulta({ onVoltar, onContinuar }) {
  ...
  const [pickerType, setPickerType] = useState(""); // "especialidade", "procedimento", "localidade"
...
const openPicker = (type, currentValue) => {
  ...
  const options = (() => {
    if (type === "especialidade") return ESPECIALIDADES;
    if (type === "procedimento") return PROCEDIMENTOS;
    if (type === "localidade") return LOCALIDADES;
    if (type === "medico") return MEDICOS;
    return [];
  })();

  setTempValue(currentValue || options[0] || ""); // <- pega o primeiro valor se vazio
  setPickerOpen(true);
};
  ...
  if (pickerType === "medico") return MEDICOS;
  return [];
};
