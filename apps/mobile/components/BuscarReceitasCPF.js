...
export default class BuscarReceitasCPF extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      cpf: "",
  ...
  chipStyle = (venc) => (
    this.isVencida(venc)
      ? { bg: "rgba(255,99,132,0.15)", border: "rgba(255,99,132,0.45)", text: "Vencida" }
      : { bg: "rgba(75,192,192,0.15)", border: "rgba(75,192,192,0.45)", text: "Válida" }
  );
  ...
  buscar = async () => {
    try {
      ...
      if (cpfN.length !== 11) return Alert.alert("CPF inválido", "Informe 11 dígitos.");
      this.setState({ carregando: true });

      const [pacSemPrefixo, pacComPrefixo, recStr] = await Promise.all([
        AsyncStorage.getItem(cpfN),
        AsyncStorage.getItem(`PAC_${cpfN}`),
        AsyncStorage.getItem(`REC_${cpfN}`),
      ]);

      const pacienteStr = pacSemPrefixo || pacComPrefixo;
      const paciente = pacienteStr ? JSON.parse(pacienteStr) : null;
      const receitas = recStr ? JSON.parse(recStr) : [];
      ...
      if (!paciente && (!receitas || receitas.length === 0)) {
        Alert.alert("Não encontrado", "Nenhum paciente ou receita para este CPF.");
      }
    } catch (e) {
      console.error(e);
  ...
  limpar = () => {
    this.setState({ cpf: "", paciente: null, receitas: [], carregando: false, buscou: false });
  };

  abrirPdf = (rec) => {
    if (rec?.pdfUri) {
      if (this.props.onVisualizarPdf) this.props.onVisualizarPdf(rec);
      else Alert.alert("Abrir PDF", "Implemente onVisualizarPdf para abrir o arquivo.");
    } else {
      Alert.alert("Sem anexo", "Esta receita não possui PDF.");
    }
  };
  ...
  handleCriarReceita = () => {
    ...
    if (cpfN.length !== 11) return Alert.alert("CPF inválido", "Valide o CPF do paciente.");
    if (this.props.onCriarReceita) this.props.onCriarReceita({ cpf: cpfN, paciente });
    else Alert.alert("Ação", `Navegar para tela CriarReceitas (CPF ${cpfN}).`);
  };
  ...
  renderPaciente() {
    ...
    return (
      <View className="section" style={[styles.section, { marginTop: 8 }]}>
  ...
  renderReceitas() {
    const { receitas, buscou, paciente } = this.state;
    if (!paciente) return null;

    return (
      <View style={[styles.section, { marginTop: 16 }]}>
        <Text style={styles.sectionTitle}>Receitas {receitas?.length ? `(${receitas.length})` : ""}</Text>

        {(!receitas || receitas.length === 0) && buscou ? (
          <Text style={styles.emptyTxt}>Nenhuma receita encontrada para este CPF.</Text>
        ) : (
          receitas.map((r, idx) => {
            const chip = this.chipStyle(r.vencimento);
            return (
              <View key={r.id || idx} style={styles.examCard}>
                <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 6 }}>
                  <MaterialCommunityIcons name="clipboard-text-outline" size={18} color="#eaf1ff" style={{ marginRight: 8 }} />
                  <Text style={styles.examTitle}>{r.titulo || "Receita"}</Text>
                </View>

                <View style={styles.examRow}>
                  <Text style={styles.examLabel}>Vencimento:</Text>
                  <Text style={styles.examValue}>{r.vencimento || "—"}</Text>
                </View>

                <View style={styles.examRow}>
                  <Text style={styles.examLabel}>Status:</Text>
                  <View style={[styles.chip, { backgroundColor: chip.bg, borderColor: chip.border }]}>
                    <Text style={styles.chipTxt}>{chip.text}</Text>
                  </View>
                </View>

                {r.pdfUri ? (
                  <Pressable onPress={() => this.abrirPdf(r)} style={[styles.pdfBtn, { marginTop: 10 }]}>
                    <MaterialCommunityIcons name="file-pdf-box" size={20} color="#ffd166" style={{ marginRight: 8 }} />
                    <Text style={styles.pdfBtnTxt}>{r.pdfNome || "Abrir PDF"}</Text>
                  </Pressable>
                ) : null}
              </View>
            );
        ...
        {paciente && (
          <LinearGradient colors={["#2f6edb", "#1f4fb6"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.btn, { marginTop: 16 }]}>
            <Pressable onPress={this.handleCriarReceita} style={styles.btnPress} accessibilityRole="button">
              <Text style={styles.btnText}>Criar receita para esse CPF</Text>
            </Pressable>
          </LinearGradient>
        )}
      </View>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
          ...
          <ScrollView contentContainerStyle={styles.center} keyboardShouldPersistTaps="handled">
            <View style={styles.card}>
              <Text style={styles.title}>Buscar Receitas por CPF</Text>
...
const styles = StyleSheet.create({
  ...
  inputWrap: {
    ...
    marginBottom: 12, borderBottomWidth: 1, borderBottomColor: "rgba(214,228,255,0.4)", paddingBottom: 4,
  },
  ...
  pdfBtn: {
    ...
    borderWidth: 1, borderColor: "rgba(255,214,102,0.6)", backgroundColor: "rgba(255,214,102,0.08)",
  },
